...
def _build_playlist_name(metadata=None):
    """Create a human-friendly Spotify playlist name."""
    metadata = metadata or {}
    mood = str(metadata.get('mood') or 'Mixed').title()
    activity = str(metadata.get('activity') or 'Focus').title()
    time_of_day = str(metadata.get('time_of_day') or 'Anytime').title()
    stamp = datetime.now().strftime('%Y-%m-%d')
    return f'{mood} {activity} {time_of_day} Mix {stamp}'


def _chunk_items(items, chunk_size=100):
    """Yield fixed-size chunks for Spotify playlist add calls."""
    for start in range(0, len(items), chunk_size):
        yield items[start:start + chunk_size]


def _has_required_playlist_scopes(token_info):
    """Check that the token includes the Spotify playlist write scopes."""
    if not token_info:
        return False

    granted_scopes = set(str(token_info.get('scope') or '').split())
    required_scopes = {'playlist-modify-public', 'playlist-modify-private'}
    return required_scopes.issubset(granted_scopes)


def _get_missing_playlist_scopes(token_info):
    """Return the playlist scopes that are missing from the token."""
    granted_scopes = set(str(token_info.get('scope') or '').split()) if token_info else set()
    required_scopes = {'playlist-modify-public', 'playlist-modify-private'}
    return sorted(required_scopes - granted_scopes)


def _save_playlist_to_spotify(sp, playlist, metadata=None):
    """Create a private Spotify playlist and add generated tracks to it."""
    ...
    created_playlist = sp._post(
        'me/playlists',
        payload={
            'name': playlist_name,
            'public': False,
            'collaborative': False,
            'description': 'Generated by Music Playlist Generator.',
        },
    )
    ...
    for chunk in _chunk_items(track_uris, 100):
        sp._post(f'playlists/{playlist_id}/tracks', payload={'uris': chunk})

    return {
        'id': playlist_id,
        'name': created_playlist.get('name') or playlist_name,
        'url': (created_playlist.get('external_urls') or {}).get('spotify'),
        'uri': created_playlist.get('uri'),
...
def auth_status():
    ...
    if token_info and not _has_required_playlist_scopes(token_info):
        missing_scopes = _get_missing_playlist_scopes(token_info)
        session.pop('spotify_token_info', None)
        return jsonify({
            'authenticated': False,
            'needs_reauth': True,
            'reason': 'missing_playlist_scopes',
    ...
    if not sp:
        return jsonify({'authenticated': False, 'needs_reauth': False}), 200
...
def save_playlist():
    """Save a generated playlist to the authenticated Spotify account."""
    try:
        data = request.json or {}
        ...
        token_info = session.get('spotify_token_info')
        if not _has_required_playlist_scopes(token_info):
            session.pop('spotify_token_info', None)
            return jsonify({
                'error': 'Spotify permissions are missing for playlist saving.',
                'details': 'Reconnect Spotify and approve the playlist-modify-public and playlist-modify-private permissions.',
            }), 401
        ...
        if not sp:
            return jsonify({'error': 'Please sign in with Spotify before saving playlists.'}), 401

        if refreshed_token:
            session['spotify_token_info'] = refreshed_token

        saved_playlist = _save_playlist_to_spotify(sp, playlist, metadata=metadata)
